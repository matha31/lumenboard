// backend/mcp-server/src/server.mjs — the single definition of the Lumenboard
// MCP server: tool registrations and their input schemas. Both transports
// import this (index.mjs for stdio, http.mjs for Streamable HTTP), so the tools
// a local Claude Desktop sees and the tools a remote claude.ai connector sees
// can never drift apart.
//
// Every handler is a thin wrapper over the same function the test suite calls
// directly (see ./tools/), so a bug here cannot make a tool behave differently
// than its tests claim.
